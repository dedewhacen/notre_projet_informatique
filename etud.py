...
@app.route('/gestion_etudiants', methods=['GET', 'POST'])
def gestion_etudiants():
            ...
            try:
                cursor.execute("""
                    ...
                    SET Email = %s, 
                        ...
                        Departement = %s 
                    WHERE matricule_etd = %s
                """, (email, licence, departement, matricule))
                conn.commit()
        ...
        # Ajout d'étudiant
            ...
            try:
                ...
                hashed_pwd = bcrypt.hashpw(
                    matricule.encode('utf-8'),
                    bcrypt.gensalt()
                ).decode('utf-8')  # Conversion en string

                # Insertion sécurisée
                cursor.execute("""
                    INSERT INTO etudiant 
                    (matricule_etd, Email, Licence, Departement, pwd)
                    VALUES (%s, %s, %s, %s, %s)
                """, (matricule, email, licence, departement, hashed_pwd))
            ...
            if selected_ids:
                try:
                    ...
                    cursor.execute(f"""
                        ...
                        WHERE matricule_etd IN ({placeholders})
                    """, tuple(selected_ids))
                    conn.commit()
                    flash(f"{len(selected_ids)} étudiants supprimés", "success")
                except Exception as e:
                    conn.rollback()
    ...
    try:
        cursor.execute("""
            SELECT matricule_etd, Email, Licence, Departement 
            FROM etudiant 
            ORDER BY matricule_etd 
            LIMIT %s OFFSET %s
        """, (per_page, offset))
        columns = [col[0] for col in cursor.description]
        etudiants = [dict(zip(columns, row)) for row in cursor.fetchall()]
